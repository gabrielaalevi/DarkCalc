...
class ProcessExporterMadDM(export_v4.ProcessExporterFortranSA):
    ...
    def make_model_symbolic_link(self):
        ...
        # Modified by Andre Lessa for working with MG5 v3.5.4:
        for name in ['run.inc']:
            files.ln(pjoin(self.dir_path, 'Source',name),
                 pjoin(self.dir_path, 'include'))
	...
	# Modified by Andre Lessa:
        # Copy full Source directory
        temp_dir = os.path.join(self.mgme_dir, 'Template')
        shutil.copytree(os.path.join(temp_dir, 'LO/Source'), 
	...
	# Commented out by Andre Lessa (unnecessary after the full dir has already been copied)
    ...
    def get_dd_type(self, process):
        ...
        for coupling, value in list(orders.items()):
...
#           #Write the include file containing all the resonance locations
	...
	# Modified by Andre Lessa (to be compatible with the new run.inc)        
	# Write maxparticles.inc based on max of ME's/subprocess groups
        self.Write_maxparticles_file()
    ...
    def WriteProcessNames(self, matrix_element_list):
        ...
        annihilation_pdg = collections.defaultdict(list)   # store process PDGs
        ...
        dm2dm_pdg = collections.defaultdict(list)          # store process name
        ...
        scattering_pdg = collections.defaultdict(list)     # store process PDGs
        ...
        for me in matrix_element_list.get_matrix_elements():
            ...
            if tag == self.DM2SM:
                ...
                annihilation_pdg[ids].append(str(p.get_initial_ids())+'_'+str(p.get_final_ids()))
                ...
                dm2dm_pdg[ids].append(str(p.get_initial_ids())+'_'+str(p.get_final_ids()))
                is_line_process[ids].append(has_photon(p.get_final_ids()))
                ...
                scattering_pdg[ids].append(str(p.get_initial_ids())+'_'+str(p.get_final_ids()))
                is_line_process[ids].append(has_photon(p.get_final_ids()))
        ...
        for i,dm1 in enumerate(self.dm_particles):
            for j,dm2 in enumerate(self.dm_particles[i:],i):
                ids = self.make_ids(dm1, dm2, self.DM2SM)
                for name,pdgs,has_photon in zip(annihilation[ids],annihilation_pdg[ids],is_line_process[ids]):
                    process_counter += 1
                    fsock.writelines('process_names(%i) = \'%s\'' % (process_counter, name))
                    fsock.writelines('process_pdgs(%i) = \'%s\'' % (process_counter, pdgs))
        ...
        for i,dm1 in enumerate(self.dm_particles):
            for j,dm2 in enumerate(self.dm_particles[i:],i):
                ids = self.make_ids(dm1, dm2, self.DM2DM)
                for name,pdg,has_photon in zip(dm2dm[ids],dm2dm_pdg[ids],is_line_process[ids]):
                    process_counter += 1
                    fsock.writelines('process_names(%i) = \'%s\'\n' % (process_counter, name)) 
                    fsock.writelines('process_pdgs(%i) = \'%s\'\n' % (process_counter, pdg)) 
                    is_line_process_list.append(has_photon)
        ...
        for i,dm1 in enumerate(self.dm_particles):
            for dm2 in self.dm_particles:
                ids = self.make_ids(dm1, dm2, self.DMSM)
                for name,pdgs,has_photon in zip(scattering[ids],scattering_pdg[ids],is_line_process[ids]):
                    process_counter += 1
                    fsock.writelines('process_names(%i) = \'%s\'\n' % (process_counter, name)) 
                    fsock.writelines('process_pdgs(%i) = \'%s\'\n' % (process_counter, pdgs)) 
                    is_line_process_list.append(has_photon)
    ...
    def Write_maxparticles_file(self):
        """
        Simply method to write a dummy maxparticles.inc file
        with maxparticles = 4
        (Introduced by Andre Lessa)
        """
        maxparticles = 4 # Hardcoded
