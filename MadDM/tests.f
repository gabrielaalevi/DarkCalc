...
c output of the ode integration
          ...
          open(unit=43, file=output_file)

          write(43,fmt='(A21)') '#  x                Y'
          do x2=1, kount
            write(43,fmt='(2(ES16.8))') xp(x2), yp(x1,x2)
          enddo
          close(43)
...
c output file for Y_eq to compare with the ode integration
          ...
          open(unit=45, file=output_file)
          write(45,fmt='(A23)') '#  x                Yeq'
...
c write the results to the output file          
            write(45,fmt='(2(ES16.8))') dble(x2), Y_eq
          enddo
...
c output of the ode integration
        ...
        open(unit=46, file=output_file)

        write(46,fmt='(A21)') '#  x                Y'
        do x1=1, kount
          write(46,fmt='(2(ES16.8))') xp(x1), yp(1,x1)
        enddo
        close(46)

c output file for the Y_eq to compare with the ode integration
        output_file = 'output/odeint_canon_Yeq.txt'
        open(unit=47, file=output_file)
        write(47,fmt='(A23)') '#  x                Yeq'
...
c write the results to the output file          
          write(47,fmt='(2(ES16.8))') dble(x1), Y_eq
        enddo
