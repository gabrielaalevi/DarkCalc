...
Tvalues = np.linspace(800, 6, nsteps) #interval of temperatures for the data, in GeV
...
pnames = [['DM', 'xd', 57,1],
         ['Coannihilator', 'b1', 1000005,1]] #list of the BSM particles present on the model
...
for i in range(0, len(pnames)):
    ...
    if pnames[i][0] == 'DM':
        ...
        print('type', part, 'dof', dof, 'mass', part_mass, 'col', col_processes)
...
for i in range(0, len(comp_names)):
    comp = comp_names[i]
    print(comp.label, comp.type, comp.PDG, comp.ID, comp.in_equilibrium, comp.mass, comp.g, comp.xarray, comp.collisions, comp.decaywidth, comp.decayreactions)