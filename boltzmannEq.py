    ...
    #collision term for component i
        ...
        #loop parsing through all the collisions component i can participate in as a reagent
            ...
            if partner.type not in SM:
                dY[i] += - (1/(3 * H)) * dsdx* sigma_v *((Y[i] * Y[partner.ID]) - (comp.equilibriumyield(x, mDM) * partner.equilibriumyield(x, mDM)) * col_term)
                dY[partner.ID] += - (1/(3*H)) * dsdx * sigma_v * ((Y[i] * Y[partner.ID]) - (comp.equilibriumyield(x, mDM) * partner.equilibriumyield(x, mDM)) * col_term)
                for p in range(0, len(product)):
                    dY[(product[p])] += (1/(3 * H)) * dsdx * sigma_v * ((Y[i]*Y[partner.ID]) - (comp.equilibriumyield(x, mDM) * partner.equilibriumyield(x, mDM)) * col_term)
